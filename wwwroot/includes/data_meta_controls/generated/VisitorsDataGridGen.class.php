<?php
	 ...
	 * listing a collection of Visitors objects.  It includes
	 ...
	 * 
	 */
	class VisitorsDataGridGen extends QDataGrid {
		/**
		 ...
		 * way of adding a column to this Visitors datagrid.  The description of a column's
		 * content can be either a text string description of a simple field name
		 * in the Visitors object, or it can be any QQNode extending from QQN::Visitors().
		 * 
		 ...
		 * sorting using two fields from the Visitors object.
		 *
		 ...
		 *
		 * @param mixed $mixContent string or QQNode from Visitors
		...
		public function MetaDataBinder() {
			...
			if ($this->Paginator) {
				$this->TotalItemCount = Visitors::CountAll();
			}
			...
			// Set the DataSource to be a Query result from Visitors, given the clauses above
			$this->DataSource = Visitors::LoadAll($objClauses);
		}
		 ...
		 *
		 * Given a QQNode or a Text String, this will return a Visitors-based QQNode.
		 * It will also verify that it is a proper Visitors-based QQNode, and will throw an exception otherwise.
		 *
		...
		protected function ResolveContentItem($mixContent) {
			if ($mixContent instanceof QQNode) {
				...
				if ($mixContent->_RootTableName == 'visitors') {
					...
					throw new QCallerException('Content QQNode has a root table of "' . $mixContent->_RootTableName . '". Must be a root of "visitors".');
			} else if (is_string($mixContent)) switch ($mixContent) {
				case 'Id': return QQN::Visitors()->Id;
				case 'Ip': return QQN::Visitors()->Ip;
				default: throw new QCallerException('Simple Property not found in VisitorsDataGrid content: ' . $mixContent);
