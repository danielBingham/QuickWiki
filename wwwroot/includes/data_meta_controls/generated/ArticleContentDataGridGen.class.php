<?php
	 ...
	 * listing a collection of ArticleContent objects.  It includes
	 ...
	 * 
	 */
	class ArticleContentDataGridGen extends QDataGrid {
		/**
		 ...
		 * way of adding a column to this ArticleContent datagrid.  The description of a column's
		 * content can be either a text string description of a simple field name
		 * in the ArticleContent object, or it can be any QQNode extending from QQN::ArticleContent().
		 * 
		 ...
		 * sorting using two fields from the ArticleContent object.
		 *
		 ...
		 *
		 * @param mixed $mixContent string or QQNode from ArticleContent
		...
		public function MetaAddEditLinkColumn($strLinkUrl, $strLinkHtml = 'Edit', $strColumnTitle = 'Edit', $intArgumentType = QMetaControlArgumentType::PathInfo) {
			switch ($intArgumentType) {
				case QMetaControlArgumentType::QueryString:
					$strLinkUrl .= '?intArticleRevisionId=<?=urlencode($_ITEM->ArticleRevisionId)?>';
					break;
				case QMetaControlArgumentType::PathInfo:
					$strLinkUrl .= '/<?=urlencode($_ITEM->ArticleRevisionId)?>';
					break;
				default:
		...
		public function MetaAddEditProxyColumn(QControlProxy $pxyControl, $strLinkHtml = 'Edit', $strColumnTitle = 'Edit') {
			$strHtml = '<a href="#" <?= $_FORM->GetControl("' . $pxyControl->ControlId . '")->RenderAsEvents($_ITEM->ArticleRevisionId, false); ?>>' . $strLinkHtml . '</a>';
			$colEditColumn = new QDataGridColumn($strColumnTitle, $strHtml, 'HtmlEntities=False');
			$this->AddColumn($colEditColumn);
			return $colEditColumn;
		}
		...
		public function MetaDataBinder() {
			...
			if ($this->Paginator) {
				$this->TotalItemCount = ArticleContent::CountAll();
			}
			...
			// Set the DataSource to be a Query result from ArticleContent, given the clauses above
			$this->DataSource = ArticleContent::LoadAll($objClauses);
		}
		 ...
		 *
		 * Given a QQNode or a Text String, this will return a ArticleContent-based QQNode.
		 * It will also verify that it is a proper ArticleContent-based QQNode, and will throw an exception otherwise.
		 *
		...
		protected function ResolveContentItem($mixContent) {
			if ($mixContent instanceof QQNode) {
				...
				if ($mixContent->_RootTableName == 'article_content') {
					...
					throw new QCallerException('Content QQNode has a root table of "' . $mixContent->_RootTableName . '". Must be a root of "article_content".');
			} else if (is_string($mixContent)) switch ($mixContent) {
				case 'ArticleRevisionId': return QQN::ArticleContent()->ArticleRevisionId;
				case 'ArticleRevision': return QQN::ArticleContent()->ArticleRevision;
				case 'Content': return QQN::ArticleContent()->Content;
				default: throw new QCallerException('Simple Property not found in ArticleContentDataGrid content: ' . $mixContent);
