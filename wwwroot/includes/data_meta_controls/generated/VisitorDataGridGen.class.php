<?php
	 ...
	 * listing a collection of Visitor objects.  It includes
	 ...
	 * 
	 */
	class VisitorDataGridGen extends QDataGrid {
		/**
		 ...
		 * way of adding a column to this Visitor datagrid.  The description of a column's
		 * content can be either a text string description of a simple field name
		 * in the Visitor object, or it can be any QQNode extending from QQN::Visitor().
		 * 
		 ...
		 * sorting using two fields from the Visitor object.
		 *
		 ...
		 *
		 * @param mixed $mixContent string or QQNode from Visitor
		...
		public function MetaDataBinder() {
			...
			if ($this->Paginator) {
				$this->TotalItemCount = Visitor::CountAll();
			}
			...
			// Set the DataSource to be a Query result from Visitor, given the clauses above
			$this->DataSource = Visitor::LoadAll($objClauses);
		}
		 ...
		 *
		 * Given a QQNode or a Text String, this will return a Visitor-based QQNode.
		 * It will also verify that it is a proper Visitor-based QQNode, and will throw an exception otherwise.
		 *
		...
		protected function ResolveContentItem($mixContent) {
			if ($mixContent instanceof QQNode) {
				...
				if ($mixContent->_RootTableName == 'visitor') {
					...
					throw new QCallerException('Content QQNode has a root table of "' . $mixContent->_RootTableName . '". Must be a root of "visitor".');
			} else if (is_string($mixContent)) switch ($mixContent) {
				case 'Id': return QQN::Visitor()->Id;
				case 'Ip': return QQN::Visitor()->Ip;
				default: throw new QCallerException('Simple Property not found in VisitorDataGrid content: ' . $mixContent);
