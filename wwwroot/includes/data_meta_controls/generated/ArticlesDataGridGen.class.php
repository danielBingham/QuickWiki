<?php
	 ...
	 * listing a collection of Articles objects.  It includes
	 ...
	 * 
	 */
	class ArticlesDataGridGen extends QDataGrid {
		/**
		 ...
		 * way of adding a column to this Articles datagrid.  The description of a column's
		 * content can be either a text string description of a simple field name
		 * in the Articles object, or it can be any QQNode extending from QQN::Articles().
		 * 
		 ...
		 * sorting using two fields from the Articles object.
		 *
		 ...
		 *
		 * @param mixed $mixContent string or QQNode from Articles
		...
		public function MetaDataBinder() {
			...
			if ($this->Paginator) {
				$this->TotalItemCount = Articles::CountAll();
			}
			...
			// Set the DataSource to be a Query result from Articles, given the clauses above
			$this->DataSource = Articles::LoadAll($objClauses);
		}
		 ...
		 *
		 * Given a QQNode or a Text String, this will return a Articles-based QQNode.
		 * It will also verify that it is a proper Articles-based QQNode, and will throw an exception otherwise.
		 *
		...
		protected function ResolveContentItem($mixContent) {
			if ($mixContent instanceof QQNode) {
				...
				if ($mixContent->_RootTableName == 'articles') {
					...
					throw new QCallerException('Content QQNode has a root table of "' . $mixContent->_RootTableName . '". Must be a root of "articles".');
			} else if (is_string($mixContent)) switch ($mixContent) {
				case 'Id': return QQN::Articles()->Id;
				case 'Title': return QQN::Articles()->Title;
				case 'ArticleRevisionId': return QQN::Articles()->ArticleRevisionId;
				default: throw new QCallerException('Simple Property not found in ArticlesDataGrid content: ' . $mixContent);
