<?php
	 ...
	 * listing a collection of ArticleRevision objects.  It includes
	 ...
	 * 
	 */
	class ArticleRevisionDataGridGen extends QDataGrid {
		/**
		 ...
		 * way of adding a column to this ArticleRevision datagrid.  The description of a column's
		 * content can be either a text string description of a simple field name
		 * in the ArticleRevision object, or it can be any QQNode extending from QQN::ArticleRevision().
		 * 
		 ...
		 * sorting using two fields from the ArticleRevision object.
		 *
		 ...
		 *
		 * @param mixed $mixContent string or QQNode from ArticleRevision
		...
		public function MetaDataBinder() {
			...
			if ($this->Paginator) {
				$this->TotalItemCount = ArticleRevision::CountAll();
			}
			...
			// Set the DataSource to be a Query result from ArticleRevision, given the clauses above
			$this->DataSource = ArticleRevision::LoadAll($objClauses);
		}
		 ...
		 *
		 * Given a QQNode or a Text String, this will return a ArticleRevision-based QQNode.
		 * It will also verify that it is a proper ArticleRevision-based QQNode, and will throw an exception otherwise.
		 *
		...
		protected function ResolveContentItem($mixContent) {
			if ($mixContent instanceof QQNode) {
				...
				if ($mixContent->_RootTableName == 'article_revision') {
					...
					throw new QCallerException('Content QQNode has a root table of "' . $mixContent->_RootTableName . '". Must be a root of "article_revision".');
			} else if (is_string($mixContent)) switch ($mixContent) {
				case 'Id': return QQN::ArticleRevision()->Id;
				case 'ArticleId': return QQN::ArticleRevision()->ArticleId;
				case 'Article': return QQN::ArticleRevision()->Article;
				case 'VisitorId': return QQN::ArticleRevision()->VisitorId;
				case 'Visitor': return QQN::ArticleRevision()->Visitor;
				case 'Title': return QQN::ArticleRevision()->Title;
				case 'Message': return QQN::ArticleRevision()->Message;
				case 'Created': return QQN::ArticleRevision()->Created;
				case 'ArticleContent': return QQN::ArticleRevision()->ArticleContent;

				default: throw new QCallerException('Simple Property not found in ArticleRevisionDataGrid content: ' . $mixContent);
